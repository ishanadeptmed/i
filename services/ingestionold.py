...
OUTPUT_COLUMNS = ACTIVATION_COLUMNS + ["Compensation", "Chargeback"]
...
def run_ingestion(
    ...
    try:
        ...
        useful["serial"] = useful["serial"].astype(str).str.replace(r"\D", "", regex=True)
        useful["serial"] = useful["serial"].astype(int)

        dupes = useful["serial"].value_counts()
        dupes = dupes[dupes > 1]
        if not dupes.empty:
            logger.warning("Duplicate serial numbers found: %s", dupes.index.tolist())

        chargeback_rows = cur_callidus[
            cur_callidus["transaction_type_desc"].str.contains("charge", case=False, na=False)
        ].copy()
        ...
        compensation_map = callidus_detail.dropna(subset=["esn"]).set_index("esn")["transaction_amount"]
        ...
        if store_ids:
            ...
            logger.info("Store filter %s: %d -> %d rows", store_ids, before, len(useful))
...
def build_summary(
    ...
    logger.debug("Building summary for %04d-%02d manager=%s", year, month, manager_email)
    try:
        by_store = (
