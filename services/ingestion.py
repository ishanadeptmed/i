...
def run_ingestion(
    ...
    try:
        ...
        # =========================
        # Normalize ESN columns
        # =========================

        callidus_detail = callidus_detail.copy()
        callidus_detail["esn"] = _normalize_esn(callidus_detail["esn"])

        cur_callidus = cur_callidus.copy()
        cur_callidus["esn"] = _normalize_esn(cur_callidus["esn"])

        # =========================
        # Rebate Logic
        # from CurCallidusReport (cur_callidus)
        # =========================

        transaction_per_esn = (
            cur_callidus.groupby("esn", as_index=False)["transaction_amount"]
            .sum()
        )
